...
export const XP_DOT_CLEANSED = 0.5;
export const XP_NODE_CAPTURED = 10;
export const XP_ENEMY_KILL = 30;
export const XP_DAMAGE_CAP_PER_SECOND = 1;
...
const COMBAT_LOCKOUT_SECONDS = 2.5;
const BASE_REGEN_DELAY_SECONDS = 0.75;
const SUPPLY_LINK_RADIUS = 330;
const BASE_CAPTURE_SECONDS = 5;
const NODE_CAPTURE_SECONDS = 5;
const FRIENDLY_BOT_CAPTURE_MULTIPLIER = 0.25;
const MAX_FRIENDLY_BOTS = 5;
const BOT_SPAWN_COOLDOWN = 10;
...
export class Game {
	...
	resize(width: number, height: number) {
		...
		const shouldReset =
			this.dots.length === 0 || Math.abs(nextWidth - this.width) > 96 || Math.abs(nextHeight - this.height) > 96;
	...
	chooseUpgrade(choiceId: UpgradeChoiceId) {
		...
		this.player.shield = Math.min(
			...
			Math.max(this.player.shield, previousShield + (choice.id === "shield" ? 12 : 0)),
		);
	...
	recordFrameMetrics(frameMs: number, updateMs: number, drawMs: number, dpr: number, pixiMetrics?: PixiRenderMetrics) {
	...
	private getEnemyHealthSummary() {
		...
		return enemies
			.map((enemy) => {
				const hp = enemy.isRespawning
					? `R${enemy.respawnTimer.toFixed(0)}`
					: `${Math.round(enemy.health)}/${Math.round(enemy.shield)}`;
	...
	private getAiTargetSummary() {
		...
		return active
			.map(
				(enemy) =>
					`${enemy.id}:${enemy.mode}:${enemy.selectedTarget}:${enemy.targetScore.toFixed(1)}@${enemy.lastDecisionInterval.toFixed(1)}s`,
			)
			.join(" | ");
	}
	...
	getStats(): GameStats {
		...
		return {
			...
			playerXp: Math.floor(this.player.xp),
			playerNextLevelXp:
				this.player.level >= MAX_CORE_LEVEL ? this.getPlayerXpCap() : this.getNextLevelXp(this.player.level),
		...
		for (const dot of dots) {
			for (let row = dot.row - 2; row <= dot.row + 2; row += 1) {
				for (let col = dot.col - 2; col <= dot.col + 2; col += 1) {
					...
					if (
						neighborDistance <= spacing * 1.72 &&
						!this.blockedBetween(dot.baseX, dot.baseY, candidate.baseX, candidate.baseY)
					) {
	...
	private getNextLevelXp(level: number) {
		return (
			[0, 80, 180, 320, 500, 720, 980, 1280, 1620, 2000, Number.POSITIVE_INFINITY][clamp(level, 1, MAX_CORE_LEVEL)] ??
	...
	private createBlockers(count: number, kind: BlockerKind) {
		...
		while (blockers.filter((blocker) => blocker.kind === kind).length < count && attempts < 160) {
			...
			const tooCloseToOther = blockers.some(
				(blocker) => distance(x, y, blocker.x, blocker.y) < radius + blocker.radius + 70,
			);
	...
	private updateCoreTimers(dt: number) {
		...
		for (const enemy of this.enemies) {
			...
			if (!enemy.isRespawning) {
				this.maybeLevelEnemy(
					enemy,
					dt *
						this.getDifficultySettings().enemyXpRate *
						(0.55 + this.nodes.filter((node) => node.owner === "enemy").length * 0.12),
				);
				continue;
			}
	...
	private updateBases(dt: number) {
		for (const base of this.bases) {
			...
			const playerInside =
				!this.player.isRespawning &&
				this.player.active &&
				distance(this.player.x, this.player.y, base.x, base.y) <
					base.radius + this.player.collisionRadius + capturePadding;
	...
	private updateCoreRecovery(agent: Agent, dt: number) {
		...
		agent.recoveryState = baseRegen
			? "regenBase"
			: regen.health > 0 || regen.shield > 0
				? "regenOwnTerritory"
				: "noSupply";
	...
	private shouldRefreshFog(
		framePulse: boolean,
		x: number,
		y: number,
		lastReveal: { x: number; y: number; ready: boolean },
	) {
		if (!lastReveal.ready) {
			return true;
		}
	...
	private updateCurrentVisibility(
		agent: Agent,
		visibility: Float32Array,
		ids: number[],
		radiusScale = FOG_REVEAL_SCALE,
	) {
	...
	private isPlayerVisibleToEnemy(_enemy: Agent) {
		...
		if (
			distance(_enemy.x, _enemy.y, this.player.x, this.player.y) <=
			_enemy.visionRadius * 1.45 + this.player.bodyRadius
		) {
			return true;
		}
	...
	private updateAgents(dt: number) {
		...
		if (
			...
			distance(this.player.x, this.player.y, this.destination.x, this.destination.y) < 12
		) {
			this.destination.active = false;
		}
	}
	...
	private chooseFriendlyBotTarget(bot: Agent) {
		...
		bot.goalType = best.label.includes("recover")
			? "recoverAtBase"
			: best.label.includes("enemy-core")
				? "attackPlayer"
				: "captureNeutralBase";
	...
	private moveAgent(agent: Agent, dt: number) {
		...
		const next = this.clampToArena(intendedX, intendedY, agent.collisionRadius + 8);
	...
	private chooseEnemyMode(enemy: Agent) {
		...
		const attackChance =
			...
			(playerWeak ? 1.9 : 1);
		const shouldAttack =
			playerKnown && allowed.includes("attack") && Math.random() < attackChance && this.elapsedSeconds > 18;
	...
	private chooseEnemyTarget(enemy: Agent) {
		...
		const addCandidate = (x: number, y: number, label: string, goal: AIGoalType, value: number) => {
			...
			const friendlyBaseSupport = this.nodes.some(
				(node) =>
					node.owner === "enemy" &&
					node.supplied &&
					distance(node.x, node.y, target.x, target.y) < SUPPLY_LINK_RADIUS * 0.65,
			)
				? 0.45
				: 0;
			const playerDanger =
				playerKnown &&
				distance(this.player.x, this.player.y, target.x, target.y) < this.player.combatRadius + enemy.combatRadius + 80
			...
			const difficultyBonus =
				...
				(goal === "cutSupplyLine" || goal === "defendMainBase" || goal === "recaptureLostBase")
					? 0.46
		...
		const threatenedEnemyBase =
			...
			(enemyBase.captureProgress > 0.02 ||
				distance(this.player.x, this.player.y, enemyBase.x, enemyBase.y) <
					enemyBase.radius + this.player.combatRadius + 112)
				? enemyBase
				: null;
		...
		if (
			predictedRoute &&
			(this.aiDifficulty === "hard" || (this.aiDifficulty === "medium" && predictedRoute.confidence > 0.72))
		) {
			addCandidate(
				predictedRoute.x,
				predictedRoute.y,
		...
		const forwardNodes = this.nodes
			...
			.map((node) => ({
				...
				score:
					...
					distance(this.player.x, this.player.y, node.x, node.y) /
						Math.max(1, Math.hypot(this.arena.width, this.arena.height)),
			}))
	...
	private updateNodes(dt: number) {
		...
		for (const node of this.nodes) {
			...
			const playerInside =
				...
				this.player.active &&
				distance(this.player.x, this.player.y, node.x, node.y) <
					node.radius + this.player.collisionRadius + capturePadding;
			...
			const playerCaptureRate = playerInside
				...
				: friendlyBotsInside.length > 0
					? Math.min(
							0.65,
							friendlyBotsInside.reduce((sum, bot) => sum + bot.captureRate, 0),
						)
					: 0;
				...
				if (!capturer || capturer === node.owner) {
					...
					if (node.captureProgress >= 1) {
						...
						if (capturer === "player") {
							...
							this.showHint(
								"mini-base-captured",
								"Mini-base captured. +10 max health and healing station online.",
								false,
							);
						} else {
							for (const enemy of this.getActiveEnemies()) {
	...
	private applyPulseToEnemies(pulse: ActivePulse) {
		...
		for (const enemy of this.getActiveEnemies()) {
			...
			if (pulse.kind === "shockwave" && enemy.canMove) {
				...
				const target = this.clampToArena(
					enemy.x + (dx / length) * push,
					enemy.y + (dy / length) * push,
					enemy.collisionRadius + 10,
				);
				enemy.targetX = target.x;
				enemy.targetY = target.y;
	...
	private cleanseWithPlayer(dt: number) {
		...
		for (const dot of this.dots) {
			...
			const cleansePower =
				influence * dt * DIRECT_CLEANSE_SCALE * homeBonus * wellBonus * (1 + Math.max(viscosity, localViscosity) * 0.9);
			...
			if (dot.infectionAmount > 0) {
				...
				dot.playerAmount = clamp(
					dot.playerAmount + influence * dt * DIRECT_CLEANSE_SCALE * 0.72 * homeBonus * wellBonus,
					0,
					1,
				);
			}
	...
	private spreadInfection(dt: number) {
		...
		for (let index = 0; index < this.dots.length; index += 1) {
			...
			for (const enemy of this.getActiveEnemies()) {
				enemyInfluence = Math.max(
					enemyInfluence,
					smoothstep(enemy.influenceRadius, 0, dot.distanceTo(enemy.x, enemy.y)) * enemy.intensity,
				);
			}
	...
	private rechargeShockwave(dt: number) {
		...
		const supplyBonus =
			this.player.localTerritory === "base" ? 1.45 : this.player.localTerritory === "miniBase" ? 1.22 : 1;
